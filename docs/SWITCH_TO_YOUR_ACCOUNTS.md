...
- ✅ فعّل **Email** provider
> 🚨 **نتيجة إطفاء تأكيد البريد (اقرأ هذا قبل إضافة أي مشرف):**
> لأن أي بريد يمكن التسجيل به بلا إثبات ملكية، **لا يجوز أن يمنح التسجيل
> صلاحيات إدارية تلقائياً**. لو مُنح الإشراف بالبريد تلقائياً، لاستولى أي شخص
> يعرف/يخمّن بريد المشرف العام على كل المحادثات بمجرد نداء
> `/auth/v1/signup`. لذلك:
> - `handle_new_user` **لا يمنح** أي صلاحيات (بعد إصلاح أمني).
> - الترقية **بعد التسجيل** عبر `sql/promote_admins.sql`.
> - القائمة في `is_admin_email` صارت «جرد المشرفين» لا «مصدر تفويض تلقائي».
>
> ⚠️ ولا تضع الإيميلات الحقيقية في هذا المستودع العام — القائمة تعيش في مشروعك.
